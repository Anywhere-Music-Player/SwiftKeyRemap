...
class KeyEvent: NSObject {
  ...
  func start() {
    ...
    // Both Input Monitoring and Accessibility are needed before installing the event tap.
    ...
    if !AXIsProcessTrustedWithOptions(options) || !CGPreflightListenEventAccess() {
  ...
  private func updateActiveApp(_ app: NSRunningApplication) {
    if let name = app.localizedName, let id = app.bundleIdentifier {
      ...
      NotificationCenter.default.post(name: .keyRemapAppsChanged, object: nil)
    }
  }
